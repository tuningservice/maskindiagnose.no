...
function environment(overrides = {}) {
  return {
    ...
    ALLOWED_HOSTNAMES: "maskindiagnose.no,www.maskindiagnose.no",
    MAIL_WORKER: {
      async fetch() {
...
function submission(overrides = {}) {
  ...
  body.set("message", "Volvo EC220 med feilkode");
  ...
  return new Request("https://maskindiagnose.no/api/contact", {
    method: "POST",
    body,
...
test("GET is a harmless 405 health check", async () => {
  const response = await onRequest({
    request: new Request("https://maskindiagnose.no/api/contact"),
    env: environment(),
  });
...
test("valid submission reaches the private worker", async () => {
  ...
  const response = await withTurnstile("maskindiagnose.no", () =>
    onRequest({ request: submission(), env }),
  );

  assert.equal(response.status, 200);
  assert.deepEqual(await response.json(), { ok: true });
  assert.equal(forwarded.siteId, "maskindiagnose-no");
  assert.equal(forwarded.name, "Testkunde");
  assert.equal(forwarded.email, "kunde@example.com");
  assert.equal(forwarded.message, "Volvo EC220 med feilkode");
});
